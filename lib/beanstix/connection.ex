defmodule Beanstix.Connection do
  ...
  alias Beanstix.Utils
  alias Beanstix.Connection.Receiver
  alias Beanstix.Connection.SharedState
  ...
  defstruct [
    # The TCP socket that holds the connection to Beanstalkd
    socket: nil,
  ...
  def start_link(beanstix_opts, connection_opts) do
    {beanstix_opts, connection_opts} = Utils.sanitize_starting_opts(beanstix_opts, connection_opts)
    Connection.start_link(__MODULE__, beanstix_opts, connection_opts)
  end
  ...
  @spec pipeline(GenServer.server, [Beanstix.command], timeout) ::
  ...
  def connect(info, state) do
    case Utils.connect(state.opts) do
      {:ok, socket} ->
        ...
        # If this is a reconnection attempt, log that we successfully reconnected.
        if info == :backoff do
          log(state, :reconnection, ["Reconnected to Beanstalkd (", Utils.format_host(state), ?)])
        end
      ...
      {:error, reason} ->
        log state, :failed_connection, [
          "Failed to connect to Beanstalkd (", Utils.format_host(state), "): ",
          Beanstix.format_error(reason),
        ]
      ...
      {:stop, reason} ->
        # {:stop, error, state} may be returned by Beanstix.Utils.connect/1 in case
  ...
  def disconnect({:error, reason} = _error, state) do
    log state, :disconnection, [
      "Disconnected from Beanstalkd (", Utils.format_host(state), "): ", Beanstix.format_error(reason),
    ]
