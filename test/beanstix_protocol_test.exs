defmodule BeanstixProtocolTest do
  ...
  test "build_commands/1" do
    ...
    cmds = [{:use, "test"}, {:put, "test", priority: 0}]
    cmd_string = "use test\r\nput 0 0 180 4\r\ntest\r\n"
    result_string = Protocol.build_commands(cmds) |> to_string()
    assert result_string == cmd_string

    cmds = [{:use, "test"}, {:put, "test", priority: 99, ttr: 300, delay: 60}]
    cmd_string = "use test\r\nput 99 60 300 4\r\ntest\r\n"
    result_string = Protocol.build_commands(cmds) |> to_string()
    assert result_string == cmd_string

    cmds = [{:put, <<1, 2, 3>>}]
    cmd_string = <<112, 117, 116, 32, 49, 48, 50, 52, 32, 48, 32, 49, 56, 48, 32, 51, 13, 10, 1, 2, 3, 13, 10>>
    result_string = Protocol.build_commands(cmds) |> to_string()
    assert result_string == cmd_string
  end
